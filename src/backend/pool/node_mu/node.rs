...
impl NodePipeLine {
    ...
    pub async fn get_conn(self: &Arc<Self>) -> BackendResult<P2MConn> {
        self.inner
            .lock()
            .await
            .lend_with(self.cfg.max_conns_limit, grow(&self, self.cfg.grow_count))
            .await
    }
    #[inline]
    #[allow(unused_must_use)]
    pub async fn recycle(self: &Arc<Self>, conn: P2MConn) {
        self.inner
            .lock()
            .await
            .recycle(self.cfg.max_conns_limit, conn)
            .await;
    }
    #[inline]
    pub async fn reonline(self: &Arc<Self>) -> BackendResult<()> {
        self.inner
            .lock()
            .await
            .reonline_with(&self.cfg.node_id, grow(&self, self.cfg.grow_count))
            .await
    }
    #[inline]
    ...
    pub async fn takeup(self: &Arc<Self>, conn: P2MConn) {
        self.inner
            .lock()
            .await
            .takeup(self.cfg.max_conns_limit, conn)
            .await;
    }
}
